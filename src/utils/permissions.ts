...
export const canUpdateTaskStatus = (userRole: USER_ROLE, userId: string, taskCreatorId: Types.ObjectId, assignee?: Types.ObjectId) => {
    if (userRole === 'owner' || userRole === 'admin') return true;
    if (userRole === 'member') {
        if (userId === taskCreatorId.toString()) return true;
        if (assignee?.toString() === userId) return true;
    }
    return false;
}
...
export const canViewProject = (userRole: USER_ROLE, userId: string, projectVisibility: IProject['visibility'], projectCreatorId: string, projectMembers: Types.ObjectId[]) => {
...
export const canDeleteTask = (userRole: USER_ROLE, userId: string, taskCreatorId: Types.ObjectId) => {
    if (userRole === 'owner' || userRole === 'admin') return true;
    if (userRole === 'member') {
        if (userId === taskCreatorId.toString()) return true;
    };
